...
(async () => {
  ...
  // Giá in trên menu (chưa VAT) → giá phải lưu (đã +8%, làm tròn LÊN 1.000đ).
  // Đặt --gia-in thì kỳ vọng đúng số in.
  const GIA_IN_NGUYEN = process.env.KIEM_GIA_IN === '1';
  const vat = (g) => (GIA_IN_NGUYEN ? g : Math.ceil((g * 1.08) / 1000) * 1000);
  const MUON = {
  ...
  for (const [ten, g] of Object.entries(MUON)) {
    const can = vat(g);
    check(`giá "${ten}": in ${g.toLocaleString('vi-VN')} → lưu ${can.toLocaleString('vi-VN')}`,
      gia[ten] === can, `đang là ${gia[ten]}`);
  }
  if (!GIA_IN_NGUYEN) {
    // Không món nào được để nguyên giá in: quên cộng VAT một món là thu thiếu món đó mãi mãi.
    const quen = mon.filter((m) => Number(m.price) % 1000 !== 0);
    check('mọi giá đều là bội số của 1.000đ', quen.length === 0,
      quen.slice(0, 5).map((m) => m.name + '=' + m.price).join(', '));
    const tongIn = 175000 + 480000 + 60000;
    const tongLuu = gia['Ba Rọi Bò Nướng Than'] + gia['Lẩu Bò Tơ 68'] + gia['Cơm Chiên Tỏi'];
    check('tổng 3 món mẫu tăng đúng khoảng 8%',
      tongLuu > tongIn * 1.079 && tongLuu < tongIn * 1.09,
      `${tongIn} → ${tongLuu} (${((tongLuu / tongIn - 1) * 100).toFixed(2)}%)`);
  }
