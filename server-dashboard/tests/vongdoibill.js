...
(async () => {
  ...
  // ════ B4' : khách sửa đơn trên bill -> quay lại B1 ═══════════════════════
  ...
  check("B4' — khách sửa được đơn khi CHƯA chốt cách trả", b4p.status === 200, `HTTP ${b4p.status} ${b4p.raw}`);
  ...
  // LUẬT 1 + 2: chỉ ĐỔI CÁCH HIỂN THỊ, tuyệt đối không xoá dữ liệu. Bản bill cũ
  // phải còn nguyên trong bảng, và phải đọc lại được qua lịch sử của tờ bill.
