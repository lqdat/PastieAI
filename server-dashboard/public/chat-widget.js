(function() {
        ...
        root.innerHTML = `
            <div class="pastie-chat-window" id="pastie-chat-window">
                <div class="pastie-chat-header">
                    ...
                    <a id="pastie-whatsapp-direct-link" href="#" target="_blank" class="pastie-whatsapp-badge-btn pastie-chat-hide" title="Chat qua WhatsApp"><i class="ri-whatsapp-fill"></i> <span>WhatsApp</span></a>
    ...
    function init() {
        ...
        // Fetch social channels (e.g. WhatsApp Direct Link)
        async function fetchProjectChannels() {
            try {
                const res = await fetch(`${CONFIG.BACKEND_URL}/api/chats/channels?projectId=${encodeURIComponent(CONFIG.PROJECT_ID)}`);
                const data = await res.json();
                if (data.success && data.whatsapp && data.whatsapp.directLink) {
                    const waLink = document.getElementById('pastie-whatsapp-direct-link');
                    if (waLink) {
                        waLink.href = data.whatsapp.directLink;
                        waLink.classList.remove('pastie-chat-hide');
                    }
                }
            } catch(e) {}
        }
