...
async function createBrandedQrPoster(imageUrl, options = {}) {
    ...
    const sloganFont = '"Playfair Display", "Be Vietnam Pro", "Segoe UI", serif';
    ...
    // 4. Khối 2: Slogan nhận diện song ngữ (SIZE BẰNG VỚI KHỐI QUÉT MÃ: 18px / 18px - FONT PLAYFAIR DISPLAY)
    ctx.fillStyle = '#c90c6c';
    ctx.font = `700 18px ${sloganFont}`;
    ctx.fillText('Không rào cản ngôn ngữ, thấu hiểu mọi khách hàng', baseW / 2, y2 + 16);

    ctx.fillStyle = '#6b5667';
    ctx.font = `600 18px ${sloganFont}`;
